...
qpnp_batt_system_temp_level_set(struct qpnp_chg_chip *chip, int lvl_sel)
{
	if (lvl_sel >= 0 && lvl_sel < chip->thermal_levels) {
		if (lvl_sel == (chip->thermal_levels - 1)) {
			...
			// if lvl ==0 - mitigation disabled totally
		        chip->therm_lvl_sel = (lvl_sel==0?0:lvl_sel+1);
	...
	pr_debug("battery, setting mitigation level : %d, voltage: %d\n", lvl_sel,qpnp_boost_vget_uv(chip));
