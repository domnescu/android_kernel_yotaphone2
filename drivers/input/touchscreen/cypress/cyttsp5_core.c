...
#define TOUCH_AVDD_EN_GPIO	57  /*Configure GPIO_57 as CYPRESS 2.8v power*/
...
MODULE_FIRMWARE(CY_FW_FRONT_FILE_NAME);
MODULE_FIRMWARE(CY_FW_FRONT_FILE_NAME);
...
//	struct workqueue_struct *wq;
	...
	int num_hid_reports;
#ifdef TTHE_TUNER_SUPPORT
	struct dentry *tthe_debugfs_dir;
	struct dentry *tthe_debugfs_file;
	u8 *tthe_buf;
	u32 tthe_buf_len;
...
#endif
#ifdef CONFIG_EPD_NOTIFIER
	struct notifier_block epd_notif;
#endif
};
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case CY_DBG_MT_LIFT_ALL:
		dev_info(dev, "%s: MT LIFT ALL (cd=%p)\n", __func__, cd);
		if (cd->sysinfo.xy_mode) {
			if (cd->sysinfo.desc.tch_header_size && cd->sysinfo.desc.tch_report_id) {
				memset(cd->sysinfo.xy_mode, 0, cd->sysinfo.desc.tch_header_size);
				cd->sysinfo.xy_mode[2] = cd->sysinfo.desc.tch_report_id;
				call_atten_cb(cd, CY_ATTEN_IRQ, CY_MODE_OPERATIONAL);
			}
		}
		break;
...
#ifdef CONFIG_EPD_NOTIFIER
static int epd_notifier_callback(struct notifier_block *self,
				 unsigned long event, void *data)
{
	int rc;
	struct epd_event *evdata = data;
	struct cyttsp5_core_data *cd = container_of(self,
						    struct cyttsp5_core_data,
						    epd_notif);
	...
	dev_dbg(cd->dev, "cyttsp5_core: %s for %s, event:%d\n", __func__,
		cd->core->id, (int)event);

	if (event == EPD_EVENT_RADIO_NOISE) {
		if (evdata->type == EPD_RADIO_NOISE_START) {
			mutex_lock(&cd->system_lock);
			rc = (cd->sleep_state != SS_SLEEP_ON) ? 1 : 0;
			mutex_unlock(&cd->system_lock);
			if (rc)
		...
		} else if (evdata->type == EPD_RADIO_NOISE_STOP) {
			mutex_lock(&cd->system_lock);
			rc = (cd->sleep_state != SS_SLEEP_ON) ? 1 : 0;
			mutex_unlock(&cd->system_lock);
			if (rc)
...
next:
...
#ifdef CONFIG_EPD_NOTIFIER
	/* listen radio noise events only for touch under EPD screen */
	if(!strcmp(cd->core->id, CY_BACK_CORE_ID)) {
		cd->epd_notif.notifier_call = epd_notifier_callback;

		rc = epd_register_client(&cd->epd_notif);
		if (rc)
			dev_err(dev, "Unable to register epd_notifier: %d\n",
				rc);
	}
#endif
	...
	cd->tthe_debugfs_dir = debugfs_create_dir(dev_name(dev), NULL);
	cd->tthe_debugfs_file = debugfs_create_file(
			CYTTSP5_TTHE_TUNER_FILE_NAME, 0644,
			cd->tthe_debugfs_dir, cd, &tthe_debugfs_fops);
#endif
...
static int cyttsp5_core_release(struct cyttsp5_core *core)
{
...
#ifdef CONFIG_EPD_NOTIFIER
	if (epd_unregister_client(&cd->epd_notif))
		dev_err(dev, "Error occurred while unregistering epd_notifier.\n");
#endif
	...
	wake_up(&cd->wait_q);
	mutex_unlock(&cd->tthe_lock);
	debugfs_remove(cd->tthe_debugfs_file);
	debugfs_remove(cd->tthe_debugfs_dir);
#endif
...
#ifdef TTHE_TUNER_SUPPORT
	.request_tthe_debugfs_dir = _cyttsp5_request_tthe_debugfs_dir,
