...
#ifdef CONFIG_TCMD
static ssize_t cyttsp5_tcmdread_status_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        struct cyttsp5_device_access_data *dad = dev_get_drvdata(dev);
        u8 val;
        ...
        mutex_lock(&dad->sysfs_lock);
        val = dad->tcmdread_status;
        mutex_unlock(&dad->sysfs_lock);

        return scnprintf(buf, CY_MAX_PRBUF_SIZE, "%d", val);
}

static DEVICE_ATTR(tcmdread_status, S_IRUSR | S_IWUSR,
		   cyttsp5_tcmdread_status_show, NULL);
#endif
...
static ssize_t cyttsp5_responsestr_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        struct cyttsp5_device_access_data *dad = dev_get_drvdata(dev);
        int i;
...
static DEVICE_ATTR(responsestr, S_IRUSR | S_IWUSR,
                   cyttsp5_responsestr_show, NULL);

#ifdef CONFIG_TCMD
static ssize_t cyttsp5_tcmdresponse_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_device_access_data *dad = dev_get_drvdata(dev);
	int i;
...
static DEVICE_ATTR(tcmdresponse, S_IRUSR | S_IWUSR,
                   cyttsp5_tcmdresponse_show, NULL);

static ssize_t cyttsp5_tcmdresponsesize_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_device_access_data *dad = dev_get_drvdata(dev);
	ssize_t num_read;
	...
	index = scnprintf(buf, CY_MAX_PRBUF_SIZE, "%d", num_read);
...
static DEVICE_ATTR(tcmdresponsesize, S_IRUSR | S_IWUSR,
		   cyttsp5_tcmdresponsesize_show, NULL);
#endif
/*
		...
		u8 *config, u16 *actual_read_len, u8 *return_buf)
{
	...
	rc = cyttsp5_request_nonhid_retrieve_panel_scan(ttsp, 0, read_offset,
			...
			return_buf);
	if (rc < 0)
		dev_err(&ttsp->dev, "%s: Retrieve2 scan data failed r=%d\n",
				__func__, rc);
	return rc;
}
...
static int cyttsp5_setup_sysfs(struct cyttsp5_device *ttsp)
{
	...
	rc = device_create_file(dev, &dev_attr_responsestr);
        if (rc) {
                dev_err(dev, "%s: Error, could not create response\n",
                                __func__);
...
#ifdef CONFIG_TCMD
	rc = device_create_file(dev, &dev_attr_tcmdresponse);
        if (rc) {
                dev_err(dev, "%s: Error, could not create response\n",
                                __func__);
                goto unregister_responsestr;
        }

	rc = device_create_file(dev, &dev_attr_tcmdresponsesize);
        if (rc) {
                dev_err(dev, "%s: Error, could not create response\n",
                                __func__);
                goto unregister_tcmdresponse;
        }

	rc = device_create_file(dev, &dev_attr_tcmdread_status);
        if (rc) {
                dev_err(dev, "%s: Error, could not create response\n",
                                __func__);
                goto unregister_tcmdresponsesize;
        }
#endif
	...
	dad->tthe_get_panel_data_debugfs = debugfs_create_file(
		...
		0644, cyttsp5_request_tthe_debugfs_dir(ttsp),
...
#ifdef CONFIG_TCMD
			goto unregister_tcmdread_status;
#else
			goto unregister_responsestr;
#endif
		}
...
//deferred_module_init_0(cyttsp5_device_access_init);
