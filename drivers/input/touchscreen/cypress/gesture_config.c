...
static gesture_config_node_t gesture_config_nodes[CONFIG_NODES_COUNT] = {
    ...
    {{{0}, 0, 0}, "keys_disabled"},
    {{{0}, 0, 0}, "palmed_screen"},
    {{{0}, 0, 0}, "touch_locked_front"},
    {{{0}, 0, 0}, "touch_locked_epd"}
};
...
static ssize_t is_front_touch_locked_show(struct kobject *kobj, struct kobj_attribute *attr,
                                  char *buf)
{
    size_t count = 0;
    int is_touch_locked = 0;
    is_touch_locked = driver_is_touch_locked(FRONT_TOUCH_ID);
    count += sprintf(buf+count, "%d", is_touch_locked);
    return count;
}

static ssize_t is_back_touch_locked_show(struct kobject *kobj, struct kobj_attribute *attr,
                                  char *buf)
{
    size_t count = 0;
    int is_touch_locked = 0;
    is_touch_locked = driver_is_touch_locked(BACK_TOUCH_ID);
    count += sprintf(buf+count, "%d", is_touch_locked);
    return count;
}

static ssize_t is_front_touch_locked_store(struct kobject *kobj, struct kobj_attribute *attr,
                                           const char *buf, size_t count)
{
    int is_touch_locked = 0;
    sscanf(buf, "%d", &is_touch_locked);
    if (is_touch_locked) {
...
static ssize_t is_back_touch_locked_store(struct kobject *kobj, struct kobj_attribute *attr,
                                          const char *buf, size_t count)
{
    int is_touch_locked = 0;
    sscanf(buf, "%d", &is_touch_locked);
    if (is_touch_locked) {
        driver_touch_lock(BACK_TOUCH_ID);
    } else {
        driver_touch_unlock(BACK_TOUCH_ID);
    }
    return count;
}
...
static void init_node_data(void) {
    ...
    ptr = &gesture_config_nodes[CONFIG_NODE_FRONT_TOUCH_LOCKED];
    ptr->attribute.show = is_front_touch_locked_show;
    ptr->attribute.store = is_front_touch_locked_store;

    ptr = &gesture_config_nodes[CONFIG_NODE_BACK_TOUCH_LOCKED];
    ptr->attribute.show = is_back_touch_locked_show;
    ptr->attribute.store = is_back_touch_locked_store;
